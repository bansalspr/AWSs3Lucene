...
public class FetchOnOpenS3IndexInput extends IndexInput implements S3IndexConfigurable {
	...
	public IndexInput slice(final String sliceDescription, final long offset, final long length) throws IOException {
		...
		final int endIndex = (int) (offset+length);

		FetchOnOpenS3IndexInput newInput = new FetchOnOpenS3IndexInput();
		newInput.data = new byte[(int) length];
		System.arraycopy(data, startIndex, newInput.data, 0, (int) length);
		newInput.length = length;
		return newInput;
	}
}
